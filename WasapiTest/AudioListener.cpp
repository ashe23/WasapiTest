...
HRESULT AudioListener::RecordAudioStream(IAudioSink* Sink, bool& Done)
{
	...
	while (!Done)
	{
		...
		while (packetLength != 0)
		{
			...
			size_t dataSize = m_pwfx->nChannels * (m_pwfx->wBitsPerSample / 8) * numFramesAvailable;
			printf("BitsPerSample: %d, Channels: %d, NumFramesAvailable: %d\n", m_pwfx->wBitsPerSample, m_pwfx->nChannels, numFramesAvailable);
			fflush(pFile);
